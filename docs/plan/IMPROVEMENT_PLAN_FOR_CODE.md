...
- **OSS借用の線引き（X1準拠）**：借用元は軒並み「LLMが数値生成・合意で自動決定・自己進化」。
  **収束/自動決定/自己改変の層は借りない。考え方・ロジックを借りて自前実装する（コード直輸入しない・D-21）。**
...
- **実績**：`_aggregate_counters` で「各審判の内在反証＝役割：claim／…」を反対論拠に追記。ライブ AAPL で
  碇が「株価：MACDヒストグラムが強気」を併記。新規テスト2。src_note も「審判の判定と内在反証のみ」に更新。

## B-5 統合機構で「全会一致でも内在不安」を可視化  〔✅ 完了 2026-05-23〕
...
- **実績**：全会一致(buy)かつ反証保持審判≥2で interpretation を「内在不安・確信度は割り引くべき」に。新規テスト2。
  ※現状は BALTHASAR のみ反証を出すため live で発火は B-3（MELCHIOR/CASPER反証）後。ロジックは前方互換で実装済。
