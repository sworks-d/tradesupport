...
| **`spec/`（正典）** | **パイプライン実装計画の単一の正典**。00_overview（全体設計）→ P1..P6/X/U（フェーズ＋タスク設計、各タスクに全体ゴール/前引き継ぎ/目的/次引き渡し/ハルシネ防止/受入）。**「何を作るか」はここが優先** |
| `IMPLEMENTATION_PLAN.md` | 詳細実装計画（WP単位の旧版・背景）。正は `spec/` |
